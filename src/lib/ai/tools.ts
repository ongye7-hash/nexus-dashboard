...
const IDEATE_PROMPT = (idea: string) => `다음 프로젝트 아이디어에 대한 설계를 해주세요:

아이디어: ${idea}

중요: 반드시 아래 순서대로 출력하세요.
...
file_structure에 프로젝트의 모든 파일을 빠짐없이 포함하세요. 1인 개발자 기준 현실적인 MVP 범위로.

**그 다음** 마크다운 설계 보고서를 작성하세요:
