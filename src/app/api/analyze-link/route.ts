...
### 6.5 이 아이디어를 따라하면 안 되는 이유 (Devil's Advocate)
이 영상의 아이디어를 그대로 실행했을 때 실패하는 가장 현실적인 시나리오 3가지.
크리에이터가 말하지 않은 숨겨진 비용, 과장된 부분, 재현 불가능한 조건을 짚어라.
"이걸 하지 말아야 할 사람"의 프로필도 명시해라.
...
### 9. 추가 조사 추천
이 아이디어를 더 깊이 검증하기 위해 봐야 할 영상/자료 3개를 추천해라.
반드시 반대 의견이나 실패 사례를 포함해라.
형식: 검색 키워드 + 왜 이걸 봐야 하는지 1줄 설명.`;
